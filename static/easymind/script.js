/* ============================================================
   easymind — script.js
   TypeForm-style reminder flow + NLP + Django REST
   ============================================================ */
...
function normalizeTime(raw) {
  // return an ISO time string HH:MM:SS whenever possible
  const cleaned = raw.trim().toLowerCase();
  const hm = cleaned.match(/^(\d{1,2})(?::(\d{2}))?\s*(am|pm)?$/i);
  if (!hm) return raw.trim();

  let hour = parseInt(hm[1], 10);
  let minute = hm[2] ? parseInt(hm[2], 10) : 0;
  const suffix = hm[3] ? hm[3].toLowerCase() : null;
  if (suffix) {
    if (suffix === 'pm' && hour < 12) hour += 12;
    if (suffix === 'am' && hour === 12) hour = 0;
  }
  if (hour === 24) hour = 0;
  if (hour > 23 || minute > 59) return raw.trim();
...
async function handleQ1() {
  ...
  state.parsedTitle = parsed.cleanTitle || val;

  // if NLP found everything, submit immediately
...
function createQuickRemindOverlay() {
  ...
  overlay.innerHTML = `
    <div id="quick-remind-bar">
      ...
      <input type="text" id="quick-remind-input" placeholder='Try "Apply for OSAP tomorrow by 5PM" — use plain English text' autocomplete="off">
...
async function handleQuickRemind() {
  ...
  status.textContent = '✦ Parsing reminder…';
  input.disabled = true;

  const nlp = parseNaturalLanguage(val);
  const parsed = {
    title: nlp.cleanTitle || val,
    date: nlp.date,
    time: nlp.time,
